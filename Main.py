...
def get_sf_data(query: str, access_token_obj: dict) -> list:
    """
    For pagation of the Rest call response.
    :param query: initial query statement
    :param access_token_obj: access token object
    :return: response data in list
    """
...
def main():
    cur_dt_utc = datetime.now(timezone('UTC'))
    cur_dt_pst = cur_dt_utc.astimezone(timezone('US/Pacific'))
    ...
    for obj_name in obj_names:
        ...
        mem_obj = LoadData.data_to_mem("")
        for item in data:
            item.pop('attributes')
            item.update([('importDTUTC', cur_dt_utc), ('importDTPST', cur_dt_pst)])
            LoadData.data_to_mem(mem_obj,Util.iterable_to_line(item))
        ...
        with LoadData.cursor_op(conn_pool) as (conn, cur):
            tbl_col_list = [i.strip(' \t\n\r') for i in configSetting.sf_data_fields[obj_name].split(",")] + ['importDTUTC','importDTPST']
            cur.execute(f"truncate table stg.{obj_name};")
            cur.copy_from(mem_obj, f'stg.{obj_name}', sep=configSetting.delimiter, null='None', columns=tbl_col_list)
